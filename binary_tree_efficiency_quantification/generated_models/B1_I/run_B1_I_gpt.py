...
# Apply patches to model namespace
...
def ode_system(t, y):
    ...
    # CRITICAL: clamp physical states
    y = np.clip(y, 0.0, 1.0)
    ...
    # prevent blowup
    clean_rates = np.clip(clean_rates, -10, 10)
...
H_alpha = np.asarray(algebraic[192]).flatten()
H_beta  = np.asarray(algebraic[193]).flatten()
