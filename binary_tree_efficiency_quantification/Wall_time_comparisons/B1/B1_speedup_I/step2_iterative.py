...
# Files
MAIN = "B1_speedup_I.txt"
MODULES = "B1_speedup_I_modules.txt"
PARAMS_CSV = "B1_speedup_I_parameters.csv"
...
# --- Core algebraic-evaluation block, parameterised on H_means vector ---
# This function is called by both the residual and by ode_rhs/compute_algebraics.
# Given (t, y, H_means), it binds H_means to the iterative unknown names and
# evaluates every algebraic equation in topo order. Returns a dict-like local
# namespace (as individual variables via return tuple would be brittle, so we
# use a simple dict).
lines.extend([
...
for i, n in enumerate(iter_unknown_names):
    ...
    # Core well-conditioned residual.
    if den_expr == '1':
        lines.append(
            f"    _num = eval({num_expr!r}, globals(), loc)"
        )
        lines.append(
            f"    _den = 1.0"
        )
        lines.append(
            f"    res[{i}] = _num - H_means[{i}]"
        )
    else:
        lines.append(
            f"    _num = eval({num_expr!r}, globals(), loc)"
        ...
        lines.append(
            f"    _den = eval({den_expr!r}, globals(), loc)"
        )
        lines.append(
            f"    res[{i}] = _num - H_means[{i}] * _den"
        )
    # Zero-flow regularization (IT-4e).
    # Anchor magnitude is decoupled from the residual's native scale (which
    # shrinks with flow). Instead we use a fixed ZERO_FLOW_ANCHOR_STRENGTH,
    # gated by v_eps/(v+v_eps)-style smoothing so the anchor vanishes at
    # nonzero flow. Picking the strength much larger than any residual at the
    # true fixed point, but much smaller than H scales, guarantees the anchor
    # dominates only when flow is near zero.
    lines.append(
        f"    _flow = abs(loc.get('{comp_base}__v', 0.0)) + "
        f"abs(loc.get('{comp_base}__v_d', 0.0))"
    )
    lines.append(
        f"    _gate = ZERO_FLOW_REG_EPS**2 / (_flow**2 + ZERO_FLOW_REG_EPS**2 + 1e-300)"
    )
    lines.append(
...
for i, (state_name, _) in enumerate(state_list):
    for ode_state, ode_rhs_expr in ode_equations:
        if ode_state == state_name:
            # Evaluate the ODE RHS inside the locals namespace so it sees the
            # freshly-computed algebraics and the solved H_means. globals() is
            # passed so module-level parameters (tau_link, etc.) resolve.
            lines.append(
                f'    dydt[{i}] = eval({ode_rhs_expr!r}, globals(), loc)'
            )
            break
lines.extend([
    '    return dydt',
...
# List all names the user probably wants back: the algebraic LHSes + iterative unknowns.
for lhs, _ in all_equations:
    lines.append(f'    out["{lhs}"] = loc["{lhs}"]')
for n in iter_unknown_names:
    lines.append(f'    out["{n}"] = loc["{n}"]')
lines.extend([
    '    return out',
    '',
])
