...
def build_array(G, inlet_vids, plan, peri_length, flip=()):
    ...
    jname = {}          # filled in traversal order once the walk has run
    ...
    # A junction takes at most two inputs and two outputs. Where the walk has
    # sent too many vessels into one, the user can nominate one to run the
    # other way; those choices are applied here, before ports are allocated.
    for vid in flip:
        try:
            u, v, k = (int(x) for x in vid.rsplit("_", 2))
        except ValueError:
    ...
    # number junctions in the order the walk reaches them, so VV_junc1 is the
    # first junction downstream of the inlet rather than whichever node id
    # happened to sort first
    seen_j = []
    for e in order:
        frm, to = direction[e]
        for n in (frm, to):
            if n in junctions and n not in jname:
                jname[n] = None
                seen_j.append(n)
    for n in junctions:
        if n not in jname:
            jname[n] = None
            seen_j.append(n)
    for i, n in enumerate(seen_j):
        jname[n] = f"VV_junc{i + 1}"
    ...
    for e in (order + [x for x in direction if x not in order]):
        ...
        def neighbour_at(node, exclude_edge):
            """What this vessel connects to at `node`.

            A junction is named directly. Otherwise the node joins exactly two
            vessels, and the connection runs vessel-to-vessel -- naming only
            junctions dropped those links silently and left the element with an
            empty inp_vessels, which read as a dangling branch.
            """
            if node in junctions:
                return jname[node]
            others = [x for x in
                      ((min(a, b), max(a, b), kk) for a, b, kk in G.edges(node, keys=True))
                      if x != exclude_edge and x in name_of]
            if len(others) == 1:
                other = others[0]
                # name the pericyte if the neighbour carries one, since that is
                # the element actually adjacent
                ovid = vid_of(*other)
                return (elem_name(other, True) if direction.get(other, (None, None))[0] == node
                        and ovid in peri_set else name_of[other])
            return ""

        upstream = neighbour_at(frm, e)
        downstream = neighbour_at(to, e)
    ...
    # junction rows, emitted in the same order they are numbered
    for n in seen_j:
        ins, outs = [], []
        for e in ports[n]["in"]:
    ...
    # Two ways a vessel can end up outside the flow. Either the walk never
    # reached it -- in which case it never becomes a row at all, and would
    # disappear from the array without a word -- or it is a row with no input
    # that was not declared an inlet. Both are reported, by vessel id, so they
    # can be shown in the viewer and judged.
    named_inlets = {name_of.get(e) for e in inlet_edges}
    referenced = set()
    for r in rows:
        referenced.update(str(r.get("out_vessels", "")).split())

    orphans = []
    for e in unreached:
        key = (min(e[0], e[1]), max(e[0], e[1]), e[2])
        orphans.append({"vessel_id": vid_of(*key), "name": "(not in array)",
                        "why": "never reached from an inlet"})
    for r in rows:
        if (r["BC_type"] != "vv_junction"
                and not str(r.get("inp_vessels", "")).strip()
                and r["name"] not in named_inlets
                and r["name"] not in referenced):
            orphans.append({"vessel_id": r.get("graph_vessel_id", ""),
                            "name": r["name"], "why": "no input, not an inlet"})

    # a VV junction offers two input ports and two output ports, and flow that
    # arrives has to be able to leave
    bad_junctions = []
    for n in seen_j:
        n_in, n_out = len(ports[n]["in"]), len(ports[n]["out"])
        if n_in > 2 or n_out > 2 or n_out == 0 or n_in == 0:
            why = []
            if n_in > 2:
                why.append(f"{n_in} inputs (max 2)")
            if n_out > 2:
                why.append(f"{n_out} outputs (max 2)")
            if n_out == 0:
                why.append("no output: flow cannot leave")
            if n_in == 0:
                why.append("no input: flow cannot arrive")
            bad_junctions.append({
                "node": n, "name": jname[n], "why": "; ".join(why),
                "in": [name_of[e] for e in ports[n]["in"]],
                "out": [name_of[e] for e in ports[n]["out"]],
                "in_ids": [vid_of(*e) for e in ports[n]["in"]],
                "out_ids": [vid_of(*e) for e in ports[n]["out"]],
            })

    return rows, name_of, direction, unreached, over, orphans, bad_junctions
...
def main():
    ...
    state = {"rows": [], "name_of": {}, "direction": {}, "hover": None,
             ...
             "selected": None, "orphans": [], "orphan_vids": set(),
             "orphan_by_label": {}, "flip": set(), "bad_j": [],
             "bad_j_by_label": {}}
    ...
    def snapshot(label):
        ...
        undo_stack.append((label, copy.deepcopy(G), set(inlet_ids),
                           set(state["peri_on"]), set(state["peri_off"])))
    ...
    def restore():
        if not undo_stack:
            say("nothing to undo")
            return
        label, snap, marks, p_on, p_off = undo_stack.pop()
        state["peri_on"] = set(p_on)
        state["peri_off"] = set(p_off)
    ...
    orphan_pick = ComboBox(choices=[], label="unreachable")
    junc_pick = ComboBox(choices=[], label="bad junction")
    ...
    def show_node_vessels(*_):
        ...
        if not state["flat"] and viewer.dims.ndisplay == 2:
            try:
                viewer.dims.set_current_step(0, int(round(float(pos_[0]))))
            except Exception:
                pass
        if state["flat"]:
    ...
    def do_split_one():
        ...
        for key in ("peri_on", "peri_off"):
            state[key] = remap_ids(G, state[key])
        rebuild_layer()
        do_flag_high()
    ...
    def peri_reason(vid):
        """Why this vessel does or does not carry a pericyte."""
        plan = state.get("plan")
        if plan is None:
            return "no pericyte plan yet"
        if vid in {v for v, _ in plan["eligible"]}:
            if vid in state["peri_on"]:
                return "yes (added by hand)"
            return "yes"
        if vid in state["peri_off"]:
            return "no — removed by hand"
        if vid in inlet_ids:
            return "no — inlets are excluded"
        L = None
        for a, b, k in G.edges(keys=True):
            if vid_of(a, b, k) == vid:
                L = edge_length(G, a, b, k)
                break
        if L is not None and L < min_len.value:
            return f"no — {L:.1f} um is under the {min_len.value:.0f} um threshold"
        return "no — not in the current plan (rebuild it?)"
    ...
    def show_info(row):
        ...
        info.value = (
            ...
            f"pericyte    {peri_reason(row.get('graph_vessel_id', ''))}\n"
    ...
    def do_sweep():
        ...
        kept = remap_ids(G, inlet_ids)
        inlet_ids.clear(); inlet_ids.update(kept)
        # manual pericyte edits are recorded against vessel ids too, and a
        # merge renames the vessel -- without this the edit silently stops
        # applying to the vessel it was made on
        for key in ("peri_on", "peri_off"):
            state[key] = remap_ids(G, state[key])
        rebuild_layer()
    ...
    def show_bad_junctions(bad):
        """List junctions whose port counts cannot work, and their vessels."""
        state["bad_j"] = bad
        labels = []
        mapping = {}
        for b in bad:
            for nm, vid, side in ([(n, i, "in") for n, i in zip(b["in"], b["in_ids"])]
                                  + [(n, i, "out") for n, i in zip(b["out"], b["out_ids"])]):
                lab = f"{b['name']}  {side}  {nm}"
                labels.append(lab)
                mapping[lab] = {"junction": b["name"], "node": b["node"],
                                "vessel": nm, "vessel_id": vid, "side": side,
                                "why": b["why"]}
        state["bad_j_by_label"] = mapping
        try:
            with junc_pick.changed.blocked():
                junc_pick.choices = []
                junc_pick.choices = labels
        except Exception:
            junc_pick.choices = []
            junc_pick.choices = labels
        if labels:
            junc_pick.value = labels[0]

    def bad_choice():
        lab = junc_pick.value if list(junc_pick.choices) else None
        return state.get("bad_j_by_label", {}).get(lab)

    def do_junc_show():
        c = bad_choice()
        if not c:
            say("no junctions with impossible ports\n\n(build the array first)")
            return
        state["selected"] = c["vessel_id"]
        recolour(c["vessel_id"])
        look_at(c["vessel_id"])
        say(f"{c['junction']}\n{c['why']}\n\n"
            f"{c['vessel']} currently runs {c['side']}\n{c['vessel_id']}")

    def do_junc_flip():
        """Make this vessel run the other way at its junction."""
        c = bad_choice()
        if not c:
            say("pick a vessel first")
            return
        vid = c["vessel_id"]
        if vid in state["flip"]:
            state["flip"].discard(vid)
            msg = f"{c['vessel']} put back to {c['side']}"
        else:
            state["flip"].add(vid)
            msg = (f"{c['vessel']} will now run "
                   f"{'out of' if c['side'] == 'in' else 'into'} {c['junction']}")
        note("flip_vessel_direction", vessel_id=vid, junction=c["junction"])
        state["rows"] = []
        table.setRowCount(0)
        say(msg + "\n\nrebuild the array to apply")

    def do_junc_delete():
        """Remove the junction by deleting the vessels that meet there."""
        c = bad_choice()
        if not c:
            say("pick a junction first")
            return
        n = c["node"]
        if n not in G:
            say("that node is already gone")
            return
        snapshot(f"delete junction {c['junction']}")
        doomed = list(G.edges(n, keys=True))
        ...
        rebuild_layer()
        do_flag_high()
        note("delete_junction", junction=c["junction"], node=n,
             vessels_removed=len(doomed))
        say(f"removed {c['junction']} and its {len(doomed)} vessel(s)\n\n"
            "recompute density, then rebuild")

    def look_at(vid):
        """Bring a vessel into view, whatever mode the viewer is in.

        Centring the camera is not enough in 2D: the vessel also has to be on
        the slice being displayed, or it simply is not drawn.
        """
        target = None
        for a, b, k in G.edges(keys=True):
            if vid_of(a, b, k) == vid:
                vox = np.asarray(G.edges[a, b, k].get("voxels", []), dtype=float)
                if vox.ndim == 2 and len(vox):
                    target = vox[len(vox) // 2]
                break
        if target is None:
            return False
        if not state["flat"] and viewer.dims.ndisplay == 2:
            try:
                viewer.dims.set_current_step(0, int(round(float(target[0]))))
            except Exception:
                pass
        c = np.array([0.0, target[1], target[2]]) if state["flat"] else target
        try:
            viewer.camera.center = tuple(float(x) for x in c)
            viewer.camera.zoom = max(viewer.camera.zoom, 3.0)
        except Exception:
            pass
        return True

    def show_orphans(orphans):
        """Draw the unreachable branches so they can be judged, not guessed at."""
        state["orphan_vids"] = {o["vessel_id"] for o in orphans if o["vessel_id"]}
        labels = [f"{o['name']}  {o['vessel_id']}" for o in orphans]
        state["orphan_by_label"] = {lab: o for lab, o in zip(labels, orphans)}
        try:
            with orphan_pick.changed.blocked():
                orphan_pick.choices = []
                orphan_pick.choices = labels
        except Exception:
            orphan_pick.choices = []
            orphan_pick.choices = labels
        if labels:
            orphan_pick.value = labels[0]
        recolour(state.get("selected"))

    def orphan_vessel():
        """The graph vessel behind the chosen entry."""
        lab = orphan_pick.value if list(orphan_pick.choices) else None
        o = state.get("orphan_by_label", {}).get(lab)
        if not o:
            return None, None
        return o["name"], o["vessel_id"]

    def do_orphan_show():
        if not state["rows"]:
            say("nothing to show yet.\n\nPress Build vessel array first —\n"
                "unreachable branches are only\nfound while building.")
            return
        nm, vid = orphan_vessel()
        if not vid:
            say("no unreachable branches\n\nevery vessel is reachable from\n"
                "the inlet, which is what you want")
            return
        state["selected"] = vid
        recolour(vid)
        found = look_at(vid)
        ...
        if row:
            show_info(row)
        say(f"{nm}\n{vid}\n\nunreachable from the inlet"
            + ("" if found else "\n\n(no geometry found for it)"))

    def do_orphan_inlet():
        """Treat this branch as a second inlet rather than an artefact."""
        nm, vid = orphan_vessel()
        if not vid:
            say("build the array first")
            return
        inlet_ids.add(vid)
        note("mark_inlet_from_orphan", vessel_id=vid, was=nm)
        say(f"{nm} marked as an inlet\n\nrebuild the array")
        recolour(state.get("selected"))

    def do_orphan_delete():
        """Remove the whole branch that hangs off this vessel."""
        nm, vid = orphan_vessel()
        if not vid:
            say("build the array first")
            return
        snapshot(f"delete orphan branch {nm}")
        reachable = {v for v in inlet_ids}
        # everything reachable from the inlets, so the branch is what is left
        import networkx as nx2
        keep_nodes = set()
        for iv in inlet_ids:
            u, v, k = (int(x) for x in iv.rsplit("_", 2))
            if G.has_edge(u, v, k):
                keep_nodes |= {u, v}
        comp = set()
        for c in nx2.connected_components(G):
            if c & keep_nodes:
                comp |= c
        doomed = [(a, b, k) for a, b, k in G.edges(keys=True)
                  if a not in comp and b not in comp]
        if not doomed:
            say(f"{nm} is in the same component as the inlet.\n\n"
                "It is unreachable because the flow direction cannot\n"
                "get there, not because it is detached. Marking it as\n"
                "an inlet is the sensible option.")
            return
        G.remove_edges_from(doomed)
        G.remove_nodes_from([x for x in list(G.nodes()) if G.degree(x) == 0])
        note("delete_orphan_branch", vessel_id=vid, removed=len(doomed))
        state["rows"] = []
        table.setRowCount(0)
        rebuild_layer()
        do_flag_high()
        say(f"removed {len(doomed)} vessel(s) not connected\nto the inlet")
    ...
    def draw_pericytes(plan, highlight=None):
        ...
        for a, b, k in G.edges(keys=True):
            ...
            if vox.ndim != 2 or len(vox) < 2:
                continue
            # The polyline is stored u -> v by node id, which agrees with the
            # flow direction only about 60% of the time. Taking a point near
            # vox[0] therefore put roughly 40% of the markers at the DOWNSTREAM
            # end -- visible as pericytes sitting on the free tips of terminal
            # vessels. Orient by the traversal before choosing the point.
            key = (min(a, b), max(a, b), k)
            d = state.get("direction", {}).get(key)
            if d is not None:
                frm = d[0]
                start = np.asarray(G.nodes[frm]["pos"], dtype=float)
                if (np.linalg.norm(vox[-1] - start)
                        < np.linalg.norm(vox[0] - start)):
                    vox = vox[::-1]
    ...
    def do_density():
        ...
        state["plan"] = plan
        draw_pericytes(plan)
        skipped_long = [(v, L) for v, L in plan["skipped"]
                        if L >= min_len.value and v not in inlet_ids
                        and v not in state["peri_off"]]
        if skipped_long:
            print(f"NOTE: {len(skipped_long)} vessel(s) are over the threshold "
                  f"but have no pericyte:")
            for v, L in skipped_long[:10]:
                print(f"   {v}  {L:.1f} um")
        state["skipped_long"] = skipped_long
        ...
        odd = len(state.get("skipped_long", []))
        manual = ""
        if state["peri_on"] or state["peri_off"]:
            manual = (f"\n\nMANUAL EDITS IN EFFECT\n"
                      f"  +{len(state['peri_on'])} added, "
                      f"-{len(state['peri_off'])} removed")
        say(f"pericytes:  {plan['n']} of {G.number_of_edges()} vessels\n"
            ...
            f"non-positive: {bad}"
            + (f"\n\n{odd} over threshold but skipped\n(listed in the terminal)"
               if odd else "")
            + manual
    ...
    def do_build():
        ...
        if state["plan"] is None:
            do_density()
        try:
            rows, name_of, direction, unreached, over, orphans, bad_j = build_array(
        ...
        show_bad_junctions(bad_j)
        # direction is only known after the walk, and the pericyte markers are
        # placed relative to it, so redraw them now
        if state["plan"] is not None:
            draw_pericytes(state["plan"], highlight=state.get("selected"))
        ...
        if bad_j:
            msg += (f"\n\n{len(bad_j)} junction(s) with impossible\nports:\n  "
                    + "\n  ".join(f"{b['name']}: {b['why']}" for b in bad_j[:3])
                    + ("\n  ..." if len(bad_j) > 3 else ""))
        if orphans:
            missing = sum(1 for o in orphans if o["name"] == "(not in array)")
            msg += (f"\n\n{len(orphans)} vessel(s) outside the flow\n"
                    f"(drawn in yellow)")
            if missing:
                msg += f"\n  {missing} left OUT of the array entirely"
            msg += "\n\ndelete them, or mark one as\nanother inlet, then rebuild"
    ...
    def _replan(msg):
        ...
        if state["rows"] and inlet_ids:
            try:
                rows, name_of, direction, unreached, over, orphans, bad_j = build_array(
                    G, inlet_ids, plan, peri_len.value, flip=state["flip"])
                if not over:
                    state.update(rows=rows, name_of=name_of, direction=direction)
                    fill_table(rows)
    ...
    def do_export():
        ...
        suggested = src.with_name(src.stem + f"_vessel_array_{stamp}"
                                  + (f"_{tag}" if tag else "") + ".csv")
        try:
            from qtpy.QtWidgets import QFileDialog
            chosen, _ = QFileDialog.getSaveFileName(
                None, "Save vessel array", str(suggested), "CSV (*.csv)")
            if not chosen:
    ...
    def recolour(hover=None):
        ...
        for vid in ids:
            ...
            elif vid in state.get("orphan_vids", set()):
                cols.append("yellow"); wids.append(3.5)
    ...
    panel = Container(widgets=[
        ...
        Label(value="--- unreachable branches ---"),
        orphan_pick,
        btn("Show it", do_orphan_show),
        btn("Mark as another inlet", do_orphan_inlet),
        btn("Delete disconnected vessels", do_orphan_delete),
        Label(value="--- junctions with bad ports ---"),
        junc_pick,
        btn("Show it", do_junc_show),
        btn("Flip this vessel's direction", do_junc_flip),
        btn("Delete this junction", do_junc_delete),
        Label(value="--- build ---"),
    ...
    legend = QLabel(
        "&nbsp;&nbsp;".join([
            ...
            swatch("yellow", "unreachable branch"),
